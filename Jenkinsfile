...
pipeline {
	agent {
		kubernetes {
			label 'docker-condocker-executor'  // all your pods will be named with this prefix, followed by a unique id
	...
	stages {
		stage('Build') {
			steps {
				script {
					docker.withRegistry("https://ghcr.io", 'condocker-github') {
